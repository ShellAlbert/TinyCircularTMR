...
static int gp_ok(FILE *gp) { return gp && !ferror(gp) && !feof(gp); }

int main(void) {
  FILE *gp;
  int i;
  channel_t channels[3] = {{"/tmp/TMR_Phase_A.fifo", 0},
                           {"/tmp/TMR_Phase_B.fifo", 0},
                           {"/tmp/TMR_Phase_C.fifo", 0}};
  long long n = 0;
  ...
  printf("open fifo success, start to plot...\n");
  while (!g_Exit) {

    // adjust xrange.
    fprintf(gp, "set xrange [%lld:%lld]\n",
            (n > SAMPLES) ? (n - SAMPLES + 1) : 1, n);
    fprintf(gp, "plot '-' title 'Phase A' with lines ls 1 \n");
